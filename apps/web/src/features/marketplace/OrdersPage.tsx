  ...
  return (
    <Link to={`/dashboard/orders/${order.id}`}>
      <Card hoverable className="flex items-center gap-4">
        {/* Image */}
        <div className="w-14 h-14 rounded-md border border-[var(--border)] overflow-hidden flex-shrink-0 bg-[var(--surface-0)]">
          {image ? (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-nav font-medium text-[var(--text-primary)] truncate">
            {order.listing?.title || 'Order'}
          </p>
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-label font-medium ${ORDER_STATUS_VARIANTS[order.status]}`}>
        ...
        <div className="text-right flex-shrink-0">
          <p className="text-nav font-semibold text-[var(--text-primary)]">
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-body text-[var(--text-secondary)]">Sign in to view your orders</p>
        ...
        <TabsContent value="buying">
          {buyerLoading ? (
            ...
            <div className="flex flex-col items-center py-12 text-center">
              <ShoppingBag size={40} className="text-[var(--border)] mb-3" />
              <p className="text-nav text-[var(--text-secondary)]">No purchases yet</p>
              <Link to="/marketplace" className="text-nav text-[var(--accent)] hover:text-[var(--accent)] mt-2">
                Browse marketplace
              </Link>
            </div>
          ) : (
        ...
        <TabsContent value="selling">
          {sellerLoading ? (
            ...
            <div className="flex flex-col items-center py-12 text-center">
              ...
              <p className="text-nav text-[var(--text-secondary)]">No sales yet</p>
              <Link to="/marketplace/sell" className="text-nav text-[var(--accent)] hover:text-[var(--accent)] mt-2">
                Start selling
              </Link>
            </div>
          ) : (
