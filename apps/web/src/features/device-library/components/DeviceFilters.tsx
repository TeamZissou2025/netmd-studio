...
function CheckboxGroup({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-tag font-medium text-[var(--text-tertiary)] uppercase tracking-wider">{label}</span>
      <div className="flex flex-col gap-0.5">
        {options.map((opt) => (
          <label
            key={opt.value}
            className="flex items-center gap-2 px-1.5 py-1 rounded-md hover:bg-[var(--surface-2)] cursor-pointer transition-colors"
          >
            <input
              type="checkbox"
            ...
            />
            <span className="text-label text-[var(--text-secondary)]">{opt.label}</span>
          </label>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <span className="text-nav font-semibold text-[var(--text-primary)]">Filters</span>
        {activeCount > 0 && (
          <button
            onClick={() =>
              onChange({
            ...
            className="text-tag text-[var(--accent)] hover:text-[var(--accent)] transition-colors"
          >
      ...
      {/* WebUSB toggle */}
      <label className="flex items-center gap-2 px-1.5 py-2 rounded-md bg-[var(--surface-1)] border border-[var(--border)] cursor-pointer">
        <input
          type="checkbox"
          ...
          className="w-3.5 h-3.5 rounded border-[var(--border)] bg-[var(--surface-0)] accent-[var(--accent)]"
        />
        <span className="text-label font-medium text-[var(--accent)]">WebUSB Compatible Only</span>
      </label>
      ...
      {/* Year Range */}
      <div className="flex flex-col gap-1.5">
        <span className="text-tag font-medium text-[var(--text-tertiary)] uppercase tracking-wider">Year Released</span>
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-label text-[var(--text-tertiary)]">to</span>
          <Input
            type="number"
            min={1992}
            max={2020}
