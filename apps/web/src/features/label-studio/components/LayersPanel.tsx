...
export function LayersPanel({ canvas, selectedObject }: LayersPanelProps) {
  ...
  return (
    <div className="p-2">
      ...
      {objects.length === 0 && (
        <p className="text-2xs text-studio-text-dim text-center py-4">No objects on canvas</p>
      )}
      <div className="space-y-0.5">
        {objects.map((obj, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`flex items-center gap-2 px-2 py-1 rounded-studio cursor-pointer transition-colors ${
                isSelected
            ...
            >
              <Icon size={12} className={isSelected ? 'text-studio-cyan' : 'text-studio-text-dim'} />
              <span className="text-2xs text-studio-text truncate flex-1">{getObjectLabel(obj)}</span>
              <button
              ...
              >
                {obj.selectable !== false ? <Unlock size={10} /> : <Lock size={10} />}
              </button>
            </div>
          );
        })}
